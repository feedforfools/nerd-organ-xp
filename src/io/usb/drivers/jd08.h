...
class JD08_MIDI : public MIDIDevice
{
    public:
        JD08_MIDI(USBHost &host) : MIDIDevice(host), bClaimed(false) {}

    protected:
        virtual bool claim(Device_t *dev, int type, const uint8_t *descriptors, uint32_t len) override
        {
            if (bClaimed) return false; // Added to prevent multiple claims => JD08 has two MIDI interfaces (TODO: needs investigation)
            if (type != 1) return false; // Claim only at interface level
            if (dev->idVendor != JD08_VENDOR_ID || dev->idProduct != JD08_PRODUCT_ID) return false; // Skip non-JD08 devices
