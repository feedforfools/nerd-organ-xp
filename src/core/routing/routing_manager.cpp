...
// #include "sources/keybed_source.h"
// #include "sinks/midi_output_sink.h"
...
void RoutingManager::onMidiMessage(PortId sourcePortId, const MusicalEvent& event)
{
    auto it = sourceMap.find(sourcePortId);
    if (it != sourceMap.end())
    {
        // TODO: Handle this better
        // it->second->fireEvent(event);
    }
}

void RoutingManager::onSwitchEvent(int switchId, bool state)
{
    Logger::log("Switch event received: " + String(switchId) + " - " + (state ? "ON" : "OFF"));
    // TODO: handle switch events
}
